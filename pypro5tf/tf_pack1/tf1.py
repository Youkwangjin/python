...
# TensorFlow 자체는 기본적으로 C++ 로 구현이 되나 파이썬, 자바, 고(GO) 등 다양한 언어를 지원한다.
# 하지만 파이썬 최우선적으로 지원하며 대부분의 편한 기능들을 파이썬 라이브러리로만 구현 되어 있어서 PYTHON으로 개발하는것을 추천한다.
...
print('GPU사용 가능'if tf.test.is_gpu_available() else 'GPU 사용 불가능')
print(tf.config.list_physical_devices('GPU'))

print('tensor : 수치용 컨테이너다. 임의의 차원을 가지는 행렬의 일반화된 모습이다. 계산 그래프 구조를 갖는다. 병렬연산이 기본')
