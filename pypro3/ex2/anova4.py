# 이원분산 분석 : 요인 복수 - 각 요인의 레벨(그룹) 복수
# 두 개의 요인에 대한 집단(독립변수) 각각이 종속변수의 영향을 주는지 검정
...
# 교호작용 가설
# 귀무 : 교호작용이 없다 (poison 종류와 독퍼짐 시간의 평균에 차이가 없다.)
# 귀무 : 교호작용이 없다 (poison 종류와 독퍼짐 시간의 평균에 차이가 있다.)
...
# 데이터의 균형설계 확인
print(data2.groupby('poison').agg(len))                        # poison 요인으로 구분된 집단 표본 수는 16으로 동일하다.
print(data2.groupby('treat').agg(len))                         # treat 요인으로 구분된 집단 표본 수는 12으로 동일하다.
print(data2.groupby(['poison', 'treat']).agg(len))             # 요인별 레벨의 표본수는 4로 동일하다.

# 모든 집단별 표본 수가 동일하므로 균형설계가 잘되었다 라고 할 수 있다.
...
# C(poison):C(treat)   6.0  0.250138  0.041690   1.874333  1.122506e-01 > 0.05 이므로 상호작용효과은 발견할 수 없다.
