...
# 여자 : 1.4 2.7 2.1 1.8 3.3 3.2 1.6 1.9 2.3 2.5 2.3 1.4 2.6 3.5 2.1 6.6 7.7 8.8 6.6 6.
...
female = [1.4, 2.7, 2.1, 1.8, 3.3, 3.2, 1.6, 1.9, 2.3, 2.5, 2.3, 1.4, 2.6, 3.5, 2.1, 6.6, 7.7, 8.8, 6.6, 6]

# 귀무: 남녀 혈관 내의 콜레스테롤 양에 차이가 없다
# 대립: 남녀 혈관 내의 콜레스테롤 양에 차이가 있다.

print(np.mean(male))     # 3.0652
print(np.mean(female))   # 3.5199
two_sample = stats.ttest_ind(male, female, equal_var=True, alternative='two-sided')  # 상동
print(two_sample)  # statistic=-0.6852589119563688, pvalue=0.49703684224362565, df=41.0

# 해석 : pvalue=0.4970 > 0.05 귀무가설 채택
...
try:
    ...
    if stats.mannwhitneyu(pay1, pay2).pvalue > 0.05:
        print('pvalue값= {}이므로 0.05보다 크기에 귀무가설(총무부, 영업부 직원의 연봉의 평균에 차이가 존재한다.) 채택'.format(
            stats.mannwhitneyu(pay1, pay2).pvalue))
    else:
        print('pvalue값= {}이므로 0.05보다 작기에 대립가설(총무부, 영업부 직원의 연봉의 평균에 차이가 존재하지 않는다.) 채택'.format(
            stats.mannwhitneyu(pay1, pay2).pvalue))
...
# 귀무 : 이 학급의 학업능력이 변화가 없다.
# 대립 : 이 학급의 학업능력이 변화가 있다.
print(np.mean(middle))  # 74.16666666666667
print(np.mean(final))   # 81.66666666666667
pair_sample = stats.ttest_rel(middle, final)
print('t-value : %.5f, p-value : %.5f'%pair_sample)

# 해석 : p-value : 0.02349 < 0.05 이므로 귀무가설 기각