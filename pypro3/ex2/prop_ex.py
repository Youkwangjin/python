...
# one-sample
# A 회사에서는 100명중의 45명이 흡연을 한다. 국가 통계에서는 35%로 알려져 있다고 할 때, 두 그룹 간 비율이 동일한가?
...
print()
# two - sample
# A회사 직원을 300명 중 100명이 햄버거를 먹었고, B회사 직원들 400명 중 170명이 햄버거를 먹었다고 할 때 비율의 동질 여부를 검정
count = np.array([100, 170])
nobs = np.array([300, 400])

z, p = proportions_ztest(count=count, nobs=nobs, value=0)
print(z)
print(p)  # 0.013675 < 0.05이므로 귀무가설 기각, 두 회사의 햄버거 취식 비율은 다르다.
