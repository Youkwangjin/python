# 추론통계 분석 중 가설 검정 : 단일표본 t
# 정규분포의(모집단) 표본에 대해 기댓값을 조사(평균차이 사용)하는 검정 방법
# 새우깡 과자 무게가 진짜 120g 이 맞는가??

# 실습 1) 어느 남성 집단의 평균 키 검정
# 귀무 : 남성의 평균키는 177.0 (모집단의 평균)
...
print(result)  # statistic=-0.22139444579394396, pvalue=0.8356282194243566
print('statistic(t값):%.5f, pvalue:%.5f'%result)

# 해석 : p-value : 0.8356 > 0.05이므로 귀무가설 채택

print('==============================')
# 실습예제 2)
# A중학교 1학년 1반 학생들의 시험결과가 담긴 파일을 읽어 처리 (국어 점수 평균 검정) student.csv
# 귀무 : 학생들의 국어 점수 평균은 80.0 이다.
# 대립 : 학생들의 국어 점수 평균은 80.0 아니다.
...
print(result2)  # statistic=-1.3321801667713216, pvalue=0.19856051824785262, df=19
print('statistic(t값):%.5f, pvalue:%.5f'%result2)

# 해석 : p-value : 0.19856 > 0.05이므로 귀무가설 채택. 학생들의 국어 점수 평균은 80.0 이다.
...
# 대림 : 여아 신생아의 몸무게는 평균이 2800(g) 보다 크다.
...
# 정규성 확인 수치
print(stats.shapiro(fdata.iloc[:, 2]))  # pvalue=0.01798 < 0.05 정규성을 만족하지 않는다.
# 정규성 확인 시각화 1
...
# pvalue=0.03926 < 0.05 이므로 귀무가설 기각, 여아 신생아의 몸무게는 평균이 2800(g) 보다 크다.
# ... 쓸 얘기가 많아야된다.
