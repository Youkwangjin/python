...
# 그러므로 병렬식 처리가 불가능해 하나의 스레드에서 일정시간 작동할 때, 다른 스레드는 작동이 멈추게 된다.
# 즉, thread의 성능이 제대로 발휘되지 않는다.
# 그래서 python은 multiprocessing 모듈로 threading 모듈과 유사한 API를 사용해 process를 지원하게 된다.
# multiprocessing은 비동기적이고 부분 작업 활동이 비결정적(무작위, 예측불가)인 경우에 효과적이다.
# 주로 네트워크 작업에서 많이 사용된다.
# multiprocessing은 대표적으로 Pool과 Process를 이용하여 하나 이상의 자식 process를 생성 한 후 병렬구조로 처리한다.
...
def func(x):
    print('값', x, '에 대한 작업 pid = ', os.getpid())  # processid
    time.sleep(1)
    return x * x
...
if __name__ == '__main__':
    ...
    p = Pool(processes=3)  # Pool(프로세스 개수)
