...
# train dataset으로 탐색적 데이터 분석(EDA)
...
print(train.isnull().sum())  # null 여부 갯수 확인
print(train.isna().sum())    # na 여부 갯수 확인

# 참고 : null이 포함된 칼럼 확인용 시각화 라이브러리 pip install missingno 를 사용한다.

print(train['datetime'].head(2))
# 연도별 월별, 일별, 시간별 대여량을 시각화하기 위해 연월일시분초 열을 추가
...
ax1.set(ylabel='Count', title='연도별 대여량')  # year 대신 Count
...
axes[1][1].set(xlabel='workingday', ylabel='Count', title='근무별 여부에 따른 대여량')
plt.show()
