# 공공데이터 중 출입국관광서비스 자료를 사용 - 국내 유료 관광지에 방문한 외국인 관광객 관련
...
def Chulbal():
    # JSON type 의 관광지 정보 읽기
    fname = '../testdata/서울관광지입장.json'
    jsonTP = json.loads(open(fname, 'r', encoding='utf-8').read())
    tour_table = pd.DataFrame(jsonTP, columns=('yyyymm', 'resNm', 'ForNum'))  # 년, 월, 관광지명, 입장객수
