...
# PCA는 기존의 변수를 조합(국어 + 영어 -> 어학)하여 서로 연관성이 없는 새로운 변수, 즉 주성분(principal component, PC)들을 만들어낸다.
# n개의 관측치와 p개의 변수로 구성된 데이터를 상관관계가 있는 k개의 변수로 구성된 데이터로 요약하는 방식.
# 이 때 요약된 변수는 기존 변수의 선형조합으로 원래 성질을 최대한 유지해야 한다.
# 선형대수 관점으로 볼 때 입력 데이터의 공분산 행렬을 고유값 분해하고 이렇게 구한 고유 백터에 입력데이터를 선형변환하는 것이다.
# 이 고유 벡터(행렬을 곱하더라도 방향이 변하지 않고 크기만 변하는 벡터)가 PCA의 주성분 벡터로서 입력 데이터의 분산이 큰 방향을 나타낸다.
...
print('차원 복귀 후 x2 : ', x2, ' ', x2.shape)   # 5.1  3.5 => 5.06  3.53
...
print('차원 복귀 후 x2 : ', x4[:3], ' ', x4.shape)
